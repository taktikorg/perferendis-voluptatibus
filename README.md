...
`createChat` interacts with the [`/v1/chat/@taktikorg/perferendis-voluptatibus`](https://platform.openai.com/docs/api-reference/chat/create) API endpoint.

- It streams the response
- It tracks the conversations

```ts
import { createChat } from "@taktikorg/perferendis-voluptatibus";
...
```ts
import { createChat, CancelledCompletionError } from "@taktikorg/perferendis-voluptatibus";
...
If you want to use `@taktikorg/perferendis-voluptatibus` library against another API endpoint that is compatible with the official API, you can do so by passing `apiUrl` parameter:
...
const chat = createChat({
  apiKey: '',
  apiUrl: 'https://ray.run/api/@taktikorg/perferendis-voluptatibus'
  model: "gpt-3.5-turbo",
});
```
